...
def main():
    ...
    ap.add_argument("--force", action="store_true", help="允许mock覆盖已存在的真训报告(默认拒绝,防误删vm-60实测)")
    args = ap.parse_args()
    ...
    # 防呆:mock(simulated=True)默认不覆盖已存在的真训报告(simulated=False),需显式--force或换--out
    try:
        _is_mock = bool(rep.get("rows") and rep["rows"][0].get("simulated"))
        _p = Path(args.out)
        if _is_mock and (not args.force) and _p.exists():
            _old = json.loads(_p.read_text(encoding="utf-8"))
            _old_rows = _old.get("rows", [])
            if any((not rr.get("simulated", True)) for rr in _old_rows):
                print(f"REFUSE overwrite真训报告: {args.out} 已含simulated=False,本次mock未写入.如确需覆盖请加--force或换--out", flush=True)
                print(json.dumps(rep, ensure_ascii=False, indent=2)[:3000], flush=True)
                print("PASS vm60_sweep(mock未落盘)", flush=True)
                return
    except SystemExit:
        raise
    except Exception as _e:
        print(f"[warn] overwrite-guard检查失败,继续写入: {_e}", flush=True)
