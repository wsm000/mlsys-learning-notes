...
| 缩小 batch（4→1） | reserved −0.93%（9.369 vs 9.457） | 吞吐 0.669×；每步样本数改变 | 本 workload 不值得 |
| 缩短 sequence（128→64） | reserved −1.04%（9.359 vs 9.457） | 吞吐 0.657×；上下文变短，初始 loss 3.2536 > 3.0038 | 本 workload 不值得 |

**两个变体为什么几乎没有省到显存**：账本实测激活+临时只有 0.0365 GiB，batch 砍到 1/4、序列砍到 1/2，能省的激活上限也就几十 MiB，相对 8.4 GiB 的 allocated 峰值完全看不见（两者峰值都与 checkpoint 持平在 8.410 GiB）。代价却很直接：吞吐都掉到约 0.66×（小矩阵打不满 GPU）。而且它们改变了工作负载——small_batch 每步只有 1 条样本、优化轨迹不同（最终 loss 2.6648，与 baseline 差 +0.060）；short_seq 上下文减半使单位 token 预测变难（初始 loss 3.2536 高于 baseline 的 3.0038）。所以这两行**不与主表比 loss**，只比显存与吞吐。

这条对整个 4.2(3) 的收口是：在“静态状态占 87%”的 workload 上，激活侧四个旋钮（缩 batch、缩序列、checkpoint、offload）全部被天花板压到 ±3% 以内，唯一有量级收益的是**压缩常驻状态**（8-bit AdamW −37.27%）。
...
- small_batch_eff1 / short_seq64 两个变体工作负载与主表不同（effective batch 1 / seq 64），只比显存与吞吐，loss 不可与主表直接比较。
- 两个变体是在主表跑完后于同一会话补跑的（见导出说明），采用的固定输入由主表同一 token 流重切块得到。
