...
## 4.4 我的推导路径：这节笔记是怎么推出来的

> 这一节记录的是**真实的学习顺序**：从现象出发，被一个接一个问题逼着往前推。中间错了三处，都标出来了——错处本身比结论更值得留档。

### 第 0 步：从一个矛盾出发

S=16384 时 naive 55.13 ms / 8544 MB，SDPA 2.385 ms / 104.6 MB，而**两者的 FLOPs 完全相同**（都是 QKᵀ + PV，O(S²d)）。既然算的一样多，差距只可能来自"算得更快"或"搬得更少"。显存差 81 倍直接否掉了前者——**瓶颈在搬，不在算**。

### 第 1 步：先造一把尺子

机器实测两个数：bf16 大矩阵乘 **141.6 TFLOP/s**、拷贝带宽 **941 GB/s**。相除得到"每搬 1 字节能顺手做多少次运算"：

$$141.6\times10^{12} \div 941\times10^{9} \approx \mathbf{150\ FLOPs/Byte}$$

这就是拐点：强度 > 150 → compute-bound；< 150 → memory-bound。

> ❌ **错处 1**：我第一遍算成了 941 ÷ 141.6 ≈ 6.6，方向反了。量纲必须和被测对象一致——算子强度是 FLOPs/Byte，阈值也必须是 FLOPs/Byte。（倒数也有意义："每 TFLOP 要搬 6.6 GB"，但它不是拐点。）

### 第 2 步：数 naive 的账

一个 score 元素 $S_{ij}$ 的一生有 **4 趟 HBM 往返**：写 S → 读 S（softmax）→ 写 P → 读 P（PV）。bf16 每趟 2 字节 → **8 字节/元素**。
它对应的运算：$\sum_{t=1}^{d} q_{it}k_{jt}$，即 $2d = 256$ FLOPs。于是

$$\text{强度} = 256 / 8 = \mathbf{32\ FLOPs/Byte} < 150 \Rightarrow \textbf{memory-bound}$$

**这就是"Prefill 受访存和中间矩阵影响"的定量形式。**

> ❌ **错处 2（最关键）**：我一度把 Q/K/V 也按"每个元素重读一遍"计费，算出 1500+ 字节/元素。错在忽略**数据复用**——一个 $q_i$ 服务 N 个 score 元素，一个 $k_j$ 被 N 个 query 共用，所以 Q/K/V 必须按 $N^2$ 分摊。
> **自检方法**：若按"每元素重读操作数"计费，任何矩阵乘都是 memory-bound；可 cuBLAS 实测跑到 141.6 TFLOP/s。模型自相矛盾，说明计费方式错了。

### 第 3 步：flash 路径的账

S/P 不再落 HBM（tile 在片上出生、消费、丢弃）→ 元素自身的搬运降到 **0**；只剩 Q/K/V 读 + O 写，共 4 个张量 $\times N \cdot d \times 2$ 字节，分摊到 $N^2$ 个元素：N=4096、d=128 时是 4 MB ÷ 1677 万 ≈ **0.25 字节/元素** → 强度 **1024 ≫ 150** → compute-bound。

**同一份数学，强度 32 → 1024。** 这就是 23 倍的来源。

### 第 4 步：那个 tile 住在哪一层

三层候选逐一排除：寄存器（线程私有，算 QKᵀ 的线程和做 softmax 的线程共享不了）、HBM（941 GB/s，正是要躲开的那个）、剩下 **shared memory / SRAM（100 KB/SM，块内共享）**。而且它**不是硬件自动给的**，是 kernel 显式声明的资源。

### 第 5 步：预算逼出 tiling

工作集 $= (3Td + T^2 + Td)\times 2$ 字节，d=128 时：

| T | 工作集 | 结果 |
|---:|---:|---|
| 64 | 72 KB | 装得下 |
| 128 | 160 KB | **爆 100 KB 预算** |

而 T 也不能太小：Q tile 数 $= N/T$，**每一块都要把所有 K/V 读一遍** → K/V 总流量 $\propto N^2 d / T$（T=16 时要多搬 512 MB）。

→ **tile size 被"SRAM 容量"（上限）和"K/V 重读代价"（下限）两头夹**，中间存在最优点。这就是 tiling 的真身。

### 第 6 步：分块把 softmax 弄坏了

两个病：① 处理第一个 K 块时 **$l$ 不完整**（分母要跨全部 key 求和，后面的 key 还没看到）；② 后面冒出更大的 score 时，之前按旧 $m$ 算的 $e^{s-m}$ **全部失效**。

### 第 7 步：手算把 online softmax 推出来

一行 4 个 score，切成两块：块 A={1,2} → $m_A=2,\ l_A=1.3679$；块 B={3,0} → $m_B=3,\ l_B=1.0498$。

新基准 $m_{new}=3$。旧的 $l_A$ 以 2 为基准，换到 3 要乘 $e^{m_A-m_{new}}=e^{-1}$：

$$1.3679\times e^{-1} + 1.0498 = 0.5032 + 1.0498 = \mathbf{1.5530}$$

与老实重算的 $e^{-2}+e^{-1}+1+e^{-3} = 1.5530$ **完全相等** → **是精确等价，不是近似**。

> ❌ **错处 3**：我第一遍把因子写成了 $e^{m_B-m_A}=e^{+1}$，方向反了。基准抬高时旧的和应该**缩小**；写成 $+1$ 会放大旧结果并导致溢出。
> 记住：$m_{new} \ge m_{old}$ ⇒ 指数上恒 ≤ 0 ⇒ **因子恒 ≤ 1**，这就是数值稳定的来源。
> 附带一个实用事实：**多数块上 $m$ 不变，因子 $=e^0=1$**，所以重标定在常见情况下几乎不花钱。

### 第 8 步：输出怎么合并

$$O_{new} = O_{old}\cdot\frac{l_{old}\,e^{m_{old}-m_{new}}}{l_{new}} + \frac{e^{S_{block}-m_{new}} \otimes V_{block}}{l_{new}}$$

物理含义是"**两个加权平均按各自分母 $l$ 加权后合并**"。

> 自测里出现的错法：每个块各自除以自己的 $l_b$ 再相加 —— 不行。**不同基准下的平均值不能直接相加**（3 个人的平均分和 5 个人的平均分不能直接相加当 8 个人的平均分），必须按 $l$ 加权。

### 第 9 步：两笔大 O 要分开记

- **空间**：状态只有 $(m, l, O)$ → 那张 $N^2$ 的中间矩阵不存了 → 中间工作集从 $O(N^2)$ 降到 $O(N\cdot d)$；
- **算力**：QKᵀ 和 PV 一次都没少 → 仍是 $O(N^2 d)$。

**FA 省的是访存和显存，不省算力。**

### 第 10 步：往上一层（4.2 的来路）

访存都解决了，为什么还有 FA2？因为**SM 喂不饱**：72 KB 工作集 vs 100 KB/SM ⇒ 每个 SM 只能挂 1 个 block ⇒ occupancy 极低、访存延迟无从掩盖。FA2 治"分工"（减少非 matmul FLOPs、序列维并行、warp 分工），A100 上到理论峰值 50–73%。

FA3 治"用上新硬件单元"：Hopper 的 **TMA**（搬运不再占线程）+ **WGMMA**（异步矩阵乘）⇒ **warp specialization**（生产者 warp 搬、消费者 warp 算），再加 FP8。硬件要求 **H100/H800、CUDA ≥ 12.3**。

FA4 面向 **Hopper + 数据中心 Blackwell（H100/B200）**，用 **CuTeDSL** 重写，做"算法与 kernel 流水线协同设计"（算力涨得比带宽快 ⇒ 要把搬运藏进计算、用多余算力换带宽），同时解决可维护性。

**按 compute capability 判定**：A100(8.0) / 4090(8.9) → FA2 风格；H100(9.0) → FA3；B200(10.0) → FA4。RTX 5090 是消费级 Blackwell（sm120），**不在 FA4 官方靶子里**，只能按 CC 回退。

> ❌ 自测里踩的坑：把 "FA3 比 FA2 快一倍" 当成错误——其实那正是 H100 上的论文数据（1.5–2.0×）。真正的错是**把一张卡的倍数搬到另一张卡**上。

### 第 11 步：再往上一层（4.3 的来路）

- **Prefix Cache 改"算多少"**：复用已 prefill 的前缀 KV，多轮场景把 $O(S_{总}^2)$ 的重算换成 $O(S_{新}\times S_{总})$ 的增量。**它命中的是"重复"，不是"长度"**——模板再长，每次都不一样就毫无用处。
- **Chunked Prefill 改"怎么算/何时算"**：把未命中 suffix 切块执行，降单次显存峰值、改善与 decode 的混排调度；**一个 FLOP 都不省**。
- 顺序：**先前缀匹配 → 再对 suffix 分块**（先知道哪段能复用，才知道哪段需要算）。
- 诊断顺序：先看 **prefill_share** 与 **prompt length / 前缀命中率**，**先证伪"重复"这个前提，再上优化**。

### 自测 5 题的结果

| 题 | 结果 | 关键结论 |
|---|---|---|
| Q1 fp32 下强度与拐点 | 半分 | 4 趟 × 4 字节 = 16 字节 → 强度 16，比 bf16 更低；fp32 算力更低 ⇒ 拐点也降到约 70–75。**错在把 Q/K/V 按元素计费** |
| Q2 tile 64→128 | ✅ | K/V 流量減半（$\propto N^2d/T$），工作集 72→160 KB 爆预算 |
| Q3 各块先除以自己的 $l_b$ 再相加 | ✅ | 不行——不同基准的平均值不能直接相加，必须按 $l$ 加权 |
| Q4 "4090 上跑 FA3，比 FA2 快一倍" | 半分 | 4090（Ada，无 TMA/WGMMA/FP8）跑不了 FA3；错的是跨硬件搬倍数，不是倍数本身 |
| Q5 RAG 的 TTFT | 半分 | 先看 prefill_share + prompt length/命中率；Prefix Cache 首选，Chunked Prefill 不省 FLOPs |
